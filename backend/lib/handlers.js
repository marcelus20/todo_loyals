...
handlers.analyze_my_request = (data, callback) => {
    acceptedMethods = ['get', 'post', 'put', 'delete'];
    if(acceptedMethods.indexOf(data.method) > -1){
        handlers._analyze_my_request[data.method](data, callback);
    }else{
        callback(405, new Err("Method not permited"));
    }
};

handlers._analyze_my_request = {};

handlers._analyze_my_request.get = (data, callback) => {
    payload = {
...
handlers._analyze_my_request.post = (data, callback) => {
    payload = {
        "path-requested":data.path,
        "query-string":data.queryStringObject,
        "host":data.headers.host,
...
handlers._analyze_my_request.put = (data, callback) => {
    payload = {
        "path-requested":data.path,
        "query-string":data.queryStringObject,
        "host":data.headers.host,
...
handlers._analyze_my_request.delete = (data, callback) => {
    payload = {
        "path-requested":data.path,
        "query-string":data.queryStringObject,
        "host":data.headers.host,
